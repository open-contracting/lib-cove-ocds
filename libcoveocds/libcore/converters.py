...
def convert_spreadsheet(upload_dir, upload_url, file_name, file_type, lib_cove_ocds_config,
    ...
    if file_type == 'csv':
        ...
        destination = os.path.join(input_name, lib_cove_ocds_config.config['root_list_path'] + '.csv')
    ...
    if lib_cove_ocds_config.config.get('hashcomments'):
        flattentool_options['default_lib_cove_ocds_config.configuration'] += ',hashcomments'

    if xml:
        flattentool_options['xml'] = True
        flattentool_options['default_lib_cove_ocds_config.configuration'] += ',IDName {}'.format(lib_cove_ocds_config.config.get('id_name', 'id')) # noqa
...
def convert_json(upload_dir, upload_url, file_name, lib_cove_ocds_config, schema_url=None, replace=False, request=None,
    ...
    if xml:
        ...
        flatten_kwargs['id_name'] = lib_cove_ocds_config.config.get('id_name', 'id')
    ...
    try:
        ...
        if lib_cove_ocds_config.config['convert_titles']:
