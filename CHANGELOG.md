...
- Remove Flatten Tool integration, including `--convert` CLI flag. Use [flatten-tool](https://flatten-tool.readthedocs.io/en/latest/), instead. For example:
...
- Remove `libcoveocds.api`. Adapt `libcoveocds.__main__`, instead.

- Remove `libcoveocds.schema.SchemaOCDS` attributes:
  ...
  - `invalid_version_argument`: Use `select_version not in libcoveocds.schema.VERSION_TAGS`.
  - `invalid_version_data`: Use `not isinstance(package["version"], str) or package["version"] not in libcoveocds.schema.VERSION_TAGS`.
  - `missing_package`: Use `"releases" not in package and "records" not in package`.
  - `schema_url`: Use `urljoin(schema_obj.base_url, "release-schema.json")`.
  - `version_choices`: Use `libcoveocds.schema.VERSION_TAGS` or `libcoveocds.schema.SchemaOCDS.base_url`.
