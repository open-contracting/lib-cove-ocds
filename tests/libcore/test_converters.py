...
import os
import json
from libcoveocds.libcore.converters import convert_json
...
def test_convert_json_1():
    ...
    output = convert_json(cove_temp_folder, "", json_filename, lib_cove_ocds_config, flatten=True)
    ...
    assert os.path.isfile(os.path.join(cove_temp_folder, "flattened", "releases.csv"))
